...
public class MidiUtils {
    ...
    /**
     * parses this message for a META tempo message and returns the tempo in
     * MPQ, or -1 if this isn't a tempo message
     */
    public static int getTempoMPQ(MidiMessage midiMsg) {
        ...
        int tempo = (msg[5] & 0xFF) | ((msg[4] & 0xFF) << 8)
                | ((msg[3] & 0xFF) << 16);
        return tempo;
    }
    ...
    public static long tick2microsecond(Sequence seq, long tick,
        ...
        if (snapshotIndex <= 0 || snapshotIndex >= cacheCount
        ...
        if (cacheCount > 0) {
            ...
            while (i < cacheCount && ticks[i] <= tick) {
                snapshotMicro += ticks2microsec(ticks[i] - ticks[i - 1],
    ...
    /**
     * Given a microsecond time, convert to tick. returns tempo at the given
     * time in cache.getCurrTempoMPQ
     */
     ...
     * @return index in track which is on or after "tick". if no entries are
     *         found that follow after tick, track.size() is returned
     */
    ...
    public static ArrayList<Integer> tick2indexEC(long tick, EventCache ec) {
        return ec.tick_to_index.get(tick) == null ? new ArrayList<Integer>(0)
                : ec.tick_to_index.get(tick);
    }

    public static long microPerTick(Sequence s, TempoCache tc) {
        return s.getMicrosecondLength() / s.getTickLength();
    }
    ...
    public static class EventCache {
        ...
        public EventCache(final Track t) {
            for (int i = 0; i < t.size(); i++) {
                ...
                events.put(i, me);
                ArrayList<Integer> tmp = tick_to_index.get(me.getTick());
                if (tmp == null) {
    ...
    public static int channel(Track t) {
        for (int i = 0; i < t.size(); i++) {
            MidiEvent me = t.get(i);
            if (me.getMessage() instanceof ShortMessage) {
                return ((ShortMessage) me.getMessage()).getChannel();
            }
        }
        return 0;
    }
